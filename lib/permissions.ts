...
const MENU_PERMISSIONS: Record<UserRole, string[]> = {
  admin: ['dashboard', 'analytics', 'projects', 'materials', 'tasks', 'task_assign', 'review', 'settings', 'user_management'],
  project_manager: ['dashboard', 'analytics', 'projects', 'materials', 'tasks', 'task_assign', 'review', 'settings', 'user_management'],
...
export function getAssignedProjects(userId: string, role: UserRole): string[] {
  ...
  // 译者：被分配了项目 1、3 和所有新创建的项目
  if (role === 'translator' && userId === 'tr001') {
    return ['*'] // 允许访问所有项目
  }

  // 质检：被分配了项目 1 和所有新创建的项目
  if (role === 'quality_checker' && userId === 'qc001') {
    return ['*'] // 允许访问所有项目
  }

  // 视频压制：被分配了项目 1 和所有新创建的项目
...
export function canAccessVariant(userId: string, role: UserRole, projectId: string, language: string): boolean {
  ...
  // 所有角色都可以访问源语言变体
  if (language.includes('源语言') || language.includes('原语言')) {
    return true
  }
