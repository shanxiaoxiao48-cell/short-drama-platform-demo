...
export interface SubtitleWithTime {
  ...
  startTime: number
  endTime: number
  trackIndex?: number // 轨道索引，用于多轨道支持
}
 ...
 * @param endTime 结束时间
 * @param existingSubtitles 现有字幕列表（包含trackIndex）
 * @returns 可用的轨道索引
 */
export function findAvailableTrackIndex(
  ...
  const subtitlesAtTime = existingSubtitles.filter(sub =>
    hasTimeOverlap({ startTime, endTime }, sub)
  )

  subtitlesAtTime.forEach(sub => {
    if (sub.trackIndex !== undefined) {
      usedTrackIndices.add(sub.trackIndex)
    }
  })
