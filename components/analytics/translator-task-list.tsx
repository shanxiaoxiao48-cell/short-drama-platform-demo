...
type SortField = "videoDuration" | "workDuration" | "completionRate" | "modificationRate" | "rejectionRate" | "spotCheckRate" | "settlementAmount" | "startedAt" | "completedAt" | "taskType"
...
export function TranslatorTaskList({ tasks }: TranslatorTaskListProps) {
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      avgSpotCheckRate: validTasks.length > 0 ? validTasks.reduce((sum, t) => sum + (t?.spotCheckRate || 0), 0) / validTasks.length : 0,
    }
  }, [filteredTasks])
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left py-3 px-2 font-medium text-muted-foreground">任务类型</th>
                ...
                <th className="text-left py-3 px-2 font-medium text-muted-foreground"><button onClick={() => handleSort("rejectionRate")} className="flex items-center gap-1 hover:text-foreground">返修修改率 {getSortIcon("rejectionRate")}</button></th>
                <th className="text-left py-3 px-2 font-medium text-muted-foreground"><button onClick={() => handleSort("spotCheckRate")} className="flex items-center gap-1 hover:text-foreground">抽检比例 {getSortIcon("spotCheckRate")}</button></th>
              ...
              {paginatedTasks.length > 0 ? (paginatedTasks.map((task) => (
                <tr key={task.id} className="border-b border-border hover:bg-muted/50">
                  ...
                  <td className="py-3 px-2">
                    <Badge variant={task.taskType === "translation" ? "default" : "secondary"} className="text-xs">
                      {task.taskType === "translation" ? "翻译" : "审校"}
                    </Badge>
                  </td>
                  ...
                  {task.taskType === "translation" ? (
                    <>
                      ...
                      <td className="py-3 px-2"><span className="text-muted-foreground">-</span></td>
                    </>
                  ) : (
                    <>
                      <td className="py-3 px-2"><span className="text-muted-foreground">-</span></td>
                      <td className="py-3 px-2"><span className="text-muted-foreground">-</span></td>
                      <td className="py-3 px-2"><span className="text-muted-foreground">-</span></td>
                      <td className="py-3 px-2 font-semibold">{task.spotCheckRate?.toFixed(1) || "-"}%</td>
                    </>
                  )}
            ...
            {paginatedTasks.length > 0 && (
              <tfoot>
                ...
                <td className="py-3 px-2" colSpan={4}>合计 ({totals.tasks}个任务)</td>
                ...
                <td className="py-3 px-2">{totals.avgSpotCheckRate.toFixed(1)}%</td>
