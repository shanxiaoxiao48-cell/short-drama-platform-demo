...
export function TranslatorDetailPage({ translatorId, onBack }: TranslatorDetailPageProps) {
  ...
  const translatorDatabase: Record<string, any> = {
    "王五": {
      ...
      languageUnitPrices: { "英语": 45, "日语": 50, "韩语": 48 },
    ...
    "张三": {
      ...
      languageUnitPrices: { "英语": 42, "西班牙语": 45 },
    ...
    "钱七": {
      ...
      languageUnitPrices: { "韩语": 48, "日语": 50 },
    ...
    "李四": {
      ...
      languageUnitPrices: { "日语": 48, "葡萄牙语": 46 },
    ...
    "赵六": {
      ...
      languageUnitPrices: { "西班牙语": 43, "葡萄牙语": 41 },
    ...
    "孙八": {
      ...
      languageUnitPrices: { "葡萄牙语": 44, "西班牙语": 46 },
    ...
    "吴十": {
      ...
      languageUnitPrices: { "法语": 50, "英语": 40 },
      genres: ["古装剧", "悬疑剧"], pricePerMinute: 39.0, status: "available" as const,
    },
    "小王": {
      id: "9", name: "小王", role: ["translator", "reviewer"] as const, rating: 9.0, grade: "S" as const,
    ...
    "小李": {
      ...
      languageUnitPrices: { "日语": 48, "法语": 50 },
    ...
    "小刘": {
      ...
      languageUnitPrices: { "英语": 46, "西班牙语": 48 },
    ...
    "小陈": {
      ...
      languageUnitPrices: { "韩语": 49, "葡萄牙语": 47 },
  ...
  const allTasks = useMemo(() => {
    ...
    // 生成符合规范的短剧ID: DJ+年月日+当日序号-语种
    const generateDramaId = (date: Date, sequence: number, language: string) => {
      const year = date.getFullYear().toString().slice(2) // 取年份后两位
      const month = String(date.getMonth() + 1).padStart(2, '0') // 月份补零
      const day = String(date.getDate()).padStart(2, '0') // 日期补零
      const seq = String(sequence).padStart(2, '0') // 序号补零
      ...
      const langCode = languageCodeMap[language] || language.substring(0, 2).toUpperCase()
      
      return `DJ${year}${month}${day}${seq}-${langCode}`
    }
    
    const tasks: any[] = []
    const dailySequences: Record<string, number> = {} // 记录每日序号
      ...
      for (let i = 0; i < Math.min(3, Math.ceil(taskCount * 0.15)); i++) {
        ...
        const spotCheckRate = 5 + getRandom() * 15
        const taskType = Math.random() > 0.5 ? "translation" : "review"
        
        // 生成任务日期
        const taskDate = new Date(baseTime)
        taskDate.setDate(taskDate.getDate() - daysAgo)
        
        // 计算当日序号
        const dateKey = taskDate.toISOString().split('T')[0]
        if (!dailySequences[dateKey]) {
          dailySequences[dateKey] = 1
        } else {
          dailySequences[dateKey]++
        }
        
        // 生成符合规范的短剧ID
        const formattedDramaId = generateDramaId(taskDate, dailySequences[dateKey], language)
        
        tasks.push({
          id: `T${translatorName}-${String(tasks.length + 1).padStart(3, '0')}`,
          dramaId: formattedDramaId, dramaName: drama.title, language,
          taskType,
  ...
  const monthlyData = useMemo(() => {
    ...
    // 翻译任务数据
    const translationTasks = completedTasks.filter(t => t.taskType === "translation")
    const avgModificationRate = translationTasks.length > 0 
      ? translationTasks.reduce((sum, t) => sum + t.modificationRate, 0) / translationTasks.length : 0
    const avgRejectionRate = translationTasks.length > 0
      ? translationTasks.reduce((sum, t) => sum + t.rejectionRate, 0) / translationTasks.length : 0
    
    // 审校任务数据
    const reviewTasks = completedTasks.filter(t => t.taskType === "review")
    const avgSpotCheckRate = reviewTasks.length > 0
      ? reviewTasks.reduce((sum, t) => sum + t.spotCheckRate, 0) / reviewTasks.length : 0
    
    return { totalTasks, totalDuration, totalWorkHours, totalAmount, avgModificationRate, avgRejectionRate, avgSpotCheckRate }
  }, [filteredTasks])
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              <div><div className="text-xs text-muted-foreground">平均返修修改率</div><div className="text-xl font-bold mt-1">{monthlyData.avgRejectionRate > 0 ? `${monthlyData.avgRejectionRate.toFixed(1)}%` : "-"}</div></div>
              <div className="col-span-2 md:col-span-3 lg:col-span-6 lg:col-start-5 lg:col-end-7">
                <div className="text-xs text-muted-foreground">平均抽检比例</div>
                <div className="text-xl font-bold mt-1">{monthlyData.avgSpotCheckRate > 0 ? `${monthlyData.avgSpotCheckRate.toFixed(1)}%` : "-"}</div>
              </div>
            </div>
          </CardContent>
        </Card>
