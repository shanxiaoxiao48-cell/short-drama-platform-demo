...
// 1. 中文不需要AI翻译环节
// 2. 其他语种需要AI翻译环节
// 3. 工作流顺序：AI翻译 → 人工翻译 → 字体调整 → 人工调整 → 成片质检 → 视频压制
// 4. 如果某个环节存在，之前的环节也必须存在
// 5. 审核驳回会产生新的人工翻译和成片质检任务，需要标注轮次
// 6. 只有人工翻译和成片质检有轮次概念，其他环节都是一轮
const allTaskList = [
  // 霸道总裁爱上我 - 英语版（已到视频压制，包含所有前置环节）
  {
    dramaId: "D001",
    drama: "霸道总裁爱上我",
    languageCode: "EN",
...
const getStageBadge = (stage: string) => {
  ...
  字体调整: { variant: "outline", className: "bg-yellow-50 text-yellow-700 border-yellow-200" },
  人工调整: { variant: "outline", className: "bg-orange-50 text-orange-700 border-orange-200" },
  成片质检: { variant: "secondary", className: "bg-sky-100 text-sky-700" },
  视频压制: { variant: "outline", className: "bg-green-50 text-green-700 border-green-200" },
  }
...
export function AnalyticsTaskProgress({
  ...
  const sortedTaskList = useMemo(() => {
    ...
    return [...filteredTaskList].sort((a, b) => {
      // 特别处理轮次字段的排序，因为显示时会将非人工翻译和成片质检的轮次显示为"-"
      if (sortField === "round") {
        const aHasRound = a.stage === "人工翻译" || a.stage === "成片质检"
        const bHasRound = b.stage === "人工翻译" || b.stage === "成片质检"
        
        // 有轮次的排在前面，没有轮次的排在后面
        if (aHasRound && !bHasRound) return -1
        if (!aHasRound && bHasRound) return 1
        if (!aHasRound && !bHasRound) return 0
  ...
  // 生成任务ID：项目ID-语种-任务缩写-轮次（仅人工翻译和成片质检显示轮次）
  // 格式：DJ26021101-EN-FY-R2
  const getTaskId = (task: (typeof allTaskList)[0], index?: number) => {
    const stageCode = stageCodeMap[task.stage] || "XX"
    
    // 生成标准化的项目ID（DJ+年月日+当日序号）
    const projectId = task.dramaId.replace(/^D/, "DJ260211") + (parseInt(task.dramaId.replace(/^D/, ""))).toString().padStart(2, "0")
    
    // 只有人工翻译和成片质检需要显示轮次
    if ((task.stage === "人工翻译" || task.stage === "成片质检") && task.round > 1) {
      return `${projectId}-${task.languageCode}-${stageCode}-R${task.round}`
    }
    return `${projectId}-${task.languageCode}-${stageCode}`
  }
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {paginatedTaskList.map((task) => {
                      ...
                      // 判断是否显示轮次（只有人工翻译和成片质检显示轮次）
                      const showRound = task.stage === "人工翻译" || task.stage === "成片质检"
                      return (
                        <tr
                          key={taskId}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        ...
                        <div>
                          <div className="text-muted-foreground mb-1">译员/审校</div>
