...
import { CheckCircle, ChevronLeft, ChevronRight, MessageSquare, Plus, History, RotateCcw, Clock, X, RefreshCw, Scissors, Merge } from "lucide-react"
              ...
              <div className="p-2 space-y-1">
                ...
                return (
                  <div
                    ...
                    ref={isSelected ? selectedRef : null}
                    className={cn(
                      "p-1.5 border rounded cursor-pointer transition-all space-y-1 group",
                      isCurrent && "border-primary/50 bg-primary/5",
                      isSelected && "border-primary bg-primary/10 shadow-sm",
                      !isCurrent && !isSelected && "border-border hover:border-primary/30"
                    )}
                  ...
                  >
                    {/* Split/Merge buttons - hover */}
                    {!isReadOnly && (onSplitSubtitle || onMergeSubtitles) && (
                      <div className="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity -mt-0.5 -mb-0.5">
                        {onSplitSubtitle && (
                          <Button variant="ghost" size="icon" className="h-5 w-5" title="拆轴"
                            onClick={(e) => { e.stopPropagation(); onSplitSubtitle(subtitle.id) }}>
                            <Scissors className="w-3 h-3" />
                          </Button>
                        )}
                        {onMergeSubtitles && (() => {
                          const idx = subtitles.indexOf(subtitle)
                          const next = subtitles[idx + 1]
                          if (!next) return null
                          return (
                            <Button variant="ghost" size="icon" className="h-5 w-5" title="合轴（与下一条）"
                              onClick={(e) => { e.stopPropagation(); onMergeSubtitles(subtitle.id, next.id) }}>
