...
interface VideoDownloadDialogProps {
  ...
  onConfirm: (selectedTypes: string[], selectedEpisodes: number[]) => void
...
export function VideoDownloadDialog({ open, onOpenChange, onConfirm, totalEpisodes = 80 }: VideoDownloadDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setSelectedTypes([])
      setSelectedEpisodes(Array.from({ length: totalEpisodes }, (_, i) => i + 1))
      setSelectAll(true)
    }
  }, [open, totalEpisodes])
  ...
  const toggleEpisode = (episode: number) => {
    if (selectedEpisodes.includes(episode)) {
      const newSelected = selectedEpisodes.filter(e => e !== episode)
      setSelectedEpisodes(newSelected)
      setSelectAll(newSelected.length === totalEpisodes)
    } else {
      const newSelected = [...selectedEpisodes, episode]
      setSelectedEpisodes(newSelected)
      setSelectAll(newSelected.length === totalEpisodes)
    }
  }
  ...
  const handleConfirm = () => {
    if (selectedTypes.length > 0 && selectedEpisodes.length > 0) {
      onConfirm(selectedTypes, selectedEpisodes)
      setSelectedTypes([])
      setSelectedEpisodes(Array.from({ length: totalEpisodes }, (_, i) => i + 1))
      setSelectAll(true)
      onOpenChange(false)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Label className="text-sm font-medium">选集</Label>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground">
                    已选 {selectedEpisodes.length} / {totalEpisodes} 集
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
              ...
              <div className="grid grid-cols-8 gap-1 p-3 rounded-lg border border-border max-h-48 overflow-y-auto">
                {Array.from({ length: totalEpisodes }, (_, i) => i + 1).map(episode => (
                  <div key={episode} className="flex items-center justify-center">
                    <Checkbox
                      id={`episode-${episode}`}
                      checked={selectedEpisodes.includes(episode)}
                      onCheckedChange={() => toggleEpisode(episode)}
                      className="sr-only"
                    />
                    <label
                      htmlFor={`episode-${episode}`}
                      className={cn(
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={selectedTypes.length === 0 || selectedEpisodes.length === 0}>
            确定
          </Button>
        </DialogFooter>
      </DialogContent>
...
interface SubtitleDownloadDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onConfirm: (selectedLanguages: string[], selectedEpisodes: number[]) => void
...
export function SubtitleDownloadDialog({
  ...
  const handleConfirm = () => {
    if (selectedLanguages.length > 0 && selectedEpisodes.length > 0) {
      onConfirm(selectedLanguages, selectedEpisodes)
      setSelectedLanguages([])
      setSelectedEpisodes(Array.from({ length: totalEpisodes }, (_, i) => i + 1))
      setSelectAll(true)
      onOpenChange(false)
      ...
      <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
        ...
        <ScrollArea className="flex-1 px-6">
          <div className="space-y-4 py-4">
            {/* 语种选择 */}
            <div>
              <Label className="text-sm font-medium mb-2 block">语种</Label>
              <div className="space-y-2">
            ...
            <div>
              ...
              <div className="grid grid-cols-8 gap-1 p-3 rounded-lg border border-border max-h-48 overflow-y-auto">
                {Array.from({ length: totalEpisodes }, (_, i) => i + 1).map(episode => (
                  <div key={episode} className="flex items-center justify-center">
                    <Checkbox
                      id={`subtitle-episode-${episode}`}
                      checked={selectedEpisodes.includes(episode)}
                      onCheckedChange={() => toggleEpisode(episode)}
                      className="sr-only"
                    />
                    <label
                      htmlFor={`subtitle-episode-${episode}`}
                      className={cn(
                        "flex items-center justify-center aspect-square rounded text-xs font-medium transition-colors border border-border",
                        selectedEpisodes.includes(episode)
                          ? "bg-primary text-primary-foreground"
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} className="bg-transparent">取消</Button>
          <Button onClick={handleConfirm} disabled={selectedLanguages.length === 0 || selectedEpisodes.length === 0}>
            确定
          </Button>
        </DialogFooter>
      </DialogContent>
...
interface ConfirmDownloadDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onConfirm: (selectedEpisodes: number[]) => void
  title: string
  description: string
  totalEpisodes?: number
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
            ...
            <div>
              ...
              <div className="grid grid-cols-8 gap-1 p-3 rounded-lg border border-border max-h-48 overflow-y-auto">
                {Array.from({ length: totalEpisodes }, (_, i) => i + 1).map(episode => (
                  <div key={episode} className="flex items-center justify-center">
                    <Checkbox
                      id={`confirm-episode-${episode}`}
                      checked={selectedEpisodes.includes(episode)}
                      onCheckedChange={() => toggleEpisode(episode)}
                      className="sr-only"
                    />
                    <label
                      htmlFor={`confirm-episode-${episode}`}
                      className={cn(
                        "flex items-center justify-center aspect-square rounded text-xs font-medium transition-colors border border-border",
                        selectedEpisodes.includes(episode)
                          ? "bg-primary text-primary-foreground"
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} className="bg-transparent">取消</Button>
          <Button onClick={handleConfirm} disabled={selectedEpisodes.length === 0}>
            确定
          </Button>
        </DialogFooter>
      </DialogContent>
