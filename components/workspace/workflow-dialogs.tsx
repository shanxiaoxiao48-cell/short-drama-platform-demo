...
export function CompletedWorkflowDialog({
  ...
  // 视频擦除流程显示不同的按钮
  const isVideoErase = workflowName === "视频擦除"
...
export function OverwriteDialog({ open, onOpenChange, onConfirm, workflowId }: OverwriteDialogProps) {
  // 根据workflowId显示不同的标题和描述
  const getDialogContent = () => {
    if (workflowId === "ai_extract") {
      return {
        title: "重新提取确认",
        description: "提取会覆盖已有结果，确认重新提取吗？"
      }
    } else if (workflowId === "video_erase") {
      return {
        title: "重新擦除确认",
        description: "擦除会覆盖已有结果，确认重新擦除吗？"
      }
    } else if (workflowId === "ai_translate") {
      return {
        title: "重新翻译确认",
        description: "翻译会覆盖已有结果，确认重新翻译吗？"
      }
    }
    return {
      title: "重新执行确认",
      description: "此操作会覆盖已有结果，确认重新执行吗？"
    }
  }
...
interface AITranslateDialogProps {
  ...
  existingLanguageVariants?: Array<{
    id: string
...
export function AITranslateDialog({ open, onOpenChange, onSubmit, existingLanguageVariants = [] }: AITranslateDialogProps) {
  const [selectedLanguages, setSelectedLanguages] = useState<string[]>([])

  // 获取已翻译的语言代码
  const existingLanguageCodes = existingLanguageVariants
    .filter(v => v.id !== "0") // 排除源语言
    .map(v => {
      // 从targetLanguage中提取语言代码（例如 "英语" -> "en"）
      const match = targetLanguages.find(l => l.name === v.targetLanguage)
      return match ? match.code : null
    })
    .filter((code): code is string => code !== null) as string[]
  ...
  const selectAll = () => {
    // 全选不包括已翻译的语言
    const availableLanguages = targetLanguages
          ...
          <ScrollArea className="h-80 border border-border rounded-lg p-4">
            <div className="grid grid-cols-3 gap-3">
              {targetLanguages.map(lang => {
                const isExisting = existingLanguageCodes.includes(lang.code)
                return (
                  <div key={lang.code} className={`flex items-center space-x-2 ${isExisting ? "opacity-50" : ""}`}>
                    <Checkbox
                      id={lang.code}
                      checked={isExisting || selectedLanguages.includes(lang.code)}
                      onCheckedChange={() => toggleLanguage(lang.code)}
                      disabled={isExisting}
                    />
                    <label htmlFor={lang.code} className={`text-sm flex-1 ${isExisting ? "text-muted-foreground" : "cursor-pointer"}`}>
                      {lang.name}
                      {isExisting && <span className="ml-1 text-xs text-muted-foreground">(已翻译)</span>}
                    </label>
                  </div>
                )
              })}
...
// Task Assignment dialog - 每个任务类型独立分配
interface TaskAssignDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSubmit: (allAssignments: Record<string, TaskAssignment[]>) => void
  ...
  initialAssignments?: Record<string, TaskAssignment[]>
}
...
const taskTypes = [
  ...
  { id: "review", name: "审校" },
  { id: "quality_check", name: "质检" },
  { id: "compress", name: "压制" },
]

const emptyAssignments: Record<string, TaskAssignment[]> = {
  translation: [],
  review: [],
  quality_check: [],
  compress: [],
...
export function TaskAssignDialog({ open, onOpenChange, onSubmit, totalEpisodes, taskType, languageVariants, initialAssignments }: TaskAssignDialogProps) {
  const [allAssignments, setAllAssignments] = useState<Record<string, TaskAssignment[]>>({ ...emptyAssignments })
  ...
  useEffect(() => {
    if (open) {
      if (initialAssignments) {
        const copy: Record<string, TaskAssignment[]> = {}
        for (const key of Object.keys(emptyAssignments)) {
          copy[key] = (initialAssignments[key] || []).map(a => ({
            languageId: a.languageId, episodes: [...a.episodes], assignee: a.assignee,
          }))
        }
        setAllAssignments(copy)
      } else {
        setAllAssignments({ translation: [], review: [], quality_check: [], compress: [] })
      }
    }
  }, [open, initialAssignments])

  const assignments = allAssignments[currentTaskType] || []
  const setAssignments = (newAssignments: TaskAssignment[]) => {
    setAllAssignments(prev => ({ ...prev, [currentTaskType]: newAssignments }))
  }

  const languages = languageVariants
    .filter(variant => !variant.targetLanguage.includes("源语言"))
    .map(variant => ({ id: variant.id, name: variant.targetLanguage, episodes: variant.totalEpisodes }))
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-5xl max-h-[90vh]">
          <DialogHeader><DialogTitle>任务分配</DialogTitle>
            <DialogDescription>请先进行AI翻译，创建翻译语言后再分配任务</DialogDescription>
          </DialogHeader>
  ...
  const getAssignedEpisodesForLanguage = (languageId: string) => {
    ...
    assignments.filter(a => a.languageId === languageId).forEach(a => a.episodes.forEach(ep => assigned.add(ep)))
    return assigned
  }

  const assignedEpisodes = getAssignedEpisodesForLanguage(currentLanguage)

  const totalTaskCount = taskTypes.length * languages.length
  const completedTaskCount = taskTypes.reduce((sum, type) => {
    const ta = allAssignments[type.id] || []
    return sum + languages.filter(lang => {
      const assigned = new Set<number>()
      ta.filter(a => a.languageId === lang.id).forEach(a => a.episodes.forEach(ep => assigned.add(ep)))
      return assigned.size >= lang.episodes && lang.episodes > 0
    }).length
  }, 0)

  const hasUnassignedEpisodes = (languageId: string) => {
    const lang = languages.find(l => l.id === languageId)
    if (!lang) return false
    return getAssignedEpisodesForLanguage(languageId).size < lang.episodes
  }

  const hasUnassignedForTaskType = (typeId: string) => {
    const ta = allAssignments[typeId] || []
    return languages.some(lang => {
      const assigned = new Set<number>()
      ta.filter(a => a.languageId === lang.id).forEach(a => a.episodes.forEach(ep => assigned.add(ep)))
      return assigned.size < lang.episodes
    })
  }

  const getSelectedEpisodes = () => {
    if (isAllSelected) {
      return Array.from({ length: currentLanguageData.episodes }, (_, i) => i + 1).filter(ep => !assignedEpisodes.has(ep))
    } else if (selectionStart !== null && selectionEnd !== null) {
      const s = Math.min(selectionStart, selectionEnd), e = Math.max(selectionStart, selectionEnd)
      const sel: number[] = []
      for (let i = s; i <= e; i++) { if (!assignedEpisodes.has(i)) sel.push(i) }
      return sel
  ...
  const handleEpisodeClick = (episode: number) => {
    ...
    if (isAllSelected) { setIsAllSelected(false); setSelectionStart(episode); setSelectionEnd(null) }
    else if (selectionStart === null) { setSelectionStart(episode); setSelectionEnd(null) }
    else if (selectionEnd === null) {
      if (episode < selectionStart) { setSelectionStart(episode); setSelectionEnd(null) }
      else { setSelectionEnd(episode) }
    } else { setSelectionStart(episode); setSelectionEnd(null) }
  }

  const handleAddAssignment = () => {
    if (!selectedAssignee || selectedEpisodes.length === 0) return
    setAssignments([...assignments, { languageId: currentLanguage, episodes: selectedEpisodes.sort((a, b) => a - b), assignee: selectedAssignee }])
    setIsAllSelected(true); setSelectionStart(null); setSelectionEnd(null); setSelectedAssignee("")
  }
  ...
  const handleResetCurrent = () => {
    ...
    setIsAllSelected(true); setSelectionStart(null); setSelectionEnd(null)
  }
  const handleSubmit = () => { onSubmit(allAssignments) }
  ...
  const formatEpisodeRange = (episodes: number[]) => {
    ...
    for (let i = 1; i < episodes.length; i++) {
      if (episodes[i] === end + 1) { end = episodes[i] }
      else { ranges.push(start === end ? `第${start}集` : `第${start}-${end}集`); start = episodes[i]; end = episodes[i] }
    }
    ranges.push(start === end ? `第${start}集` : `第${start}-${end}集`)
    return ranges.join(", ")
  }

  const handleLanguageChange = (langId: string) => { setCurrentLanguage(langId); setIsAllSelected(true); setSelectionStart(null); setSelectionEnd(null) }
  const handleTaskTypeChange = (t: string) => { setCurrentTaskType(t as any); setIsAllSelected(true); setSelectionStart(null); setSelectionEnd(null); setSelectedAssignee("") }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription>已完成 {completedTaskCount}/{totalTaskCount} 个任务（每个语言的全部集数分配完成算一个任务）</DialogDescription>
        </DialogHeader>
        <Tabs value={currentTaskType} onValueChange={handleTaskTypeChange} className="w-full mb-4">
          <TabsList className="bg-muted/50">
            {taskTypes.map(type => (
              <TabsTrigger key={type.id} value={type.id} className="relative">
                {type.name}
                {hasUnassignedForTaskType(type.id) && (<span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-red-500 rounded-full border border-background" />)}
              </TabsTrigger>
            ))}
          </TabsList>
          {taskTypes.map(type => (
            <TabsContent key={type.id} value={type.id} className="mt-0">
              <Tabs value={currentLanguage} onValueChange={handleLanguageChange} className="w-full mb-4">
                <TabsList className="bg-muted/50">
                  {languages.map(lang => {
                    const ta = allAssignments[type.id] || []
                    const asgn = new Set<number>()
                    ta.filter(a => a.languageId === lang.id).forEach(a => a.episodes.forEach(ep => asgn.add(ep)))
                    const done = asgn.size >= lang.episodes && lang.episodes > 0
                    return (
                      <TabsTrigger key={lang.id} value={lang.id} className="relative">
                        {lang.name}
                        {done ? (<span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-green-500 rounded-full border border-background" />)
                          : asgn.size < lang.episodes ? (<span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-red-500 rounded-full border border-background" />) : null}
                      </TabsTrigger>
                    )
                  })}
                </TabsList>
                        ...
                        <ScrollArea className="flex-1 border border-border rounded-lg p-3 bg-muted/30">
                          <div className="grid grid-cols-8 gap-1">
                            {Array.from({ length: lang.episodes }, (_, i) => i + 1).map(episode => {
                              const isAsgn = currentTaskType === type.id && getAssignedEpisodesForLanguage(lang.id).has(episode)
                              const isSel = currentLanguage === lang.id && currentTaskType === type.id && selectedEpisodes.includes(episode)
                              return (
                                <div key={episode}
                                  className={`aspect-square flex items-center justify-center text-xs rounded cursor-pointer transition-all select-none ${isAsgn ? 'bg-green-500/20 text-green-600 cursor-not-allowed' : isSel ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-muted border border-border'}`}
                                  onClick={() => { if (currentLanguage === lang.id && currentTaskType === type.id) handleEpisodeClick(episode); else handleLanguageChange(lang.id) }}
                                  title={isAsgn ? '已分配' : `第${episode}集`}>{episode}</div>
                              )
                            })}
                          </div>
                        </ScrollArea>
                        {currentLanguage === lang.id && currentTaskType === type.id && (
                          <div className="mt-2 text-xs text-muted-foreground">
                            {isAllSelected ? <span>当前：全选状态（点击任意集数开始选择范围）</span>
                              : selectionStart !== null && selectionEnd === null ? <span>起始：第{selectionStart}集（点击任意集数设置范围）</span>
                              : selectionStart !== null && selectionEnd !== null ? <span>范围：第{Math.min(selectionStart, selectionEnd)}-{Math.max(selectionStart, selectionEnd)}集</span> : null}
                          </div>
                        )}
                        {currentLanguage === lang.id && currentTaskType === type.id && (
                          <div className="space-y-2 pt-4 border-t mt-4">
                            ...
                            <div className="flex gap-2">
                              <Select value={selectedAssignee} onValueChange={setSelectedAssignee}>
                                <SelectTrigger className="flex-1"><SelectValue placeholder="选择人员" /></SelectTrigger>
                                <SelectContent>{personnel.map(p => (<SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>))}</SelectContent>
                              </Select>
                              <Button onClick={handleAddAssignment} disabled={!selectedAssignee || selectedEpisodes.length === 0}>添加分配</Button>
                            </div>
                            <div className="flex items-center justify-between">
                              {selectedEpisodes.length > 0 && <p className="text-xs text-muted-foreground">已选择 {selectedEpisodes.length} 集</p>}
                              <Button variant="outline" size="sm" onClick={handleResetCurrent} disabled={assignedEpisodes.size === 0}>重置当前页</Button>
                            </div>
                          </div>
                        )}
                      </div>
                      <div className="flex flex-col h-[500px]">
                        <Label className="text-sm font-semibold mb-2">已分配任务</Label>
                        <ScrollArea className="flex-1 border border-border rounded-lg p-3 bg-muted/30">
                          {assignments.filter(a => a.languageId === lang.id).length === 0 ? (
                            <div className="flex items-center justify-center h-full text-sm text-muted-foreground">暂无分配，请在左侧选择集数并分配人员</div>
                          ) : (
                            <div className="space-y-2">
                              {assignments.filter(a => a.languageId === lang.id).map((assignment, idx) => {
                                const name = personnel.find(p => p.id === assignment.assignee)?.name || "未知"
                                const realIdx = assignments.indexOf(assignment)
                                return (
                                  <div key={idx} className="flex items-start justify-between p-3 bg-background rounded-lg border border-border">
                                    <div className="flex-1 space-y-1">
                                      <div className="flex items-center gap-2">
                                        <span className="font-medium text-sm">{name}</span>
                                        <span className="text-xs text-muted-foreground">· {lang.name} ({assignment.episodes.length} 集)</span>
                                      </div>
                                      <p className="text-xs text-muted-foreground">{formatEpisodeRange(assignment.episodes)}</p>
                                    </div>
                                    <Button variant="ghost" size="sm" onClick={() => handleRemoveAssignment(realIdx)} className="text-destructive hover:text-destructive">删除</Button>
                                  </div>
                                )
                              })}
                            </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} className="relative">
            确认
            {completedTaskCount < totalTaskCount && (<span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-red-500 rounded-full border border-background" />)}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
